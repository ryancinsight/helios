...
| H-087 | Materialise the deterministic figure set for the `docs/book/` mdbook: commit seven hand-authored SVGs (photon attenuation, CT calibration, Radon sinogram profile, DVH curve, dose heatmap, MLC leaf-open sinogram, architecture stack) under `docs/book/figures/`, link them from `SUMMARY.md` / `README.md` / example chapters, add `xtask prebook` that verifies + SHA-256-hashes the figure set into a `MANIFEST.json` (byte-deterministic across runs), and add Appendix F cross-link to `repos/parity_artefacts/INDEX.md`. | [patch] | done | Codex `/root` | `docs/book/{figures,SUMMARY.md,README.md,examples/*.md}`, `xtask/{src/main.rs,src/prebook.rs,Cargo.toml}`, `repos/parity_artefacts/INDEX.md`, PM artifacts |
| H-088 | Add `prebook check-figures` subcommand that byte-scans `docs/book/SUMMARY.md` + `docs/book/README.md` for `figures/*.svg` references and asserts each one is listed in `super::prebook::FIGURE_SPECS`. Drift (orphan docs refs, or spec entries with no docs caller) exits non-zero; SSOT_IN_SYNC exits 0. Closes the SSOT drift risk H-087 review flagged by failing CI when a docs figure link is added without a matching FIGURE_SPECS entry (or vice versa). | [minor] | done | Codex `/root` | `xtask/src/{main.rs,check_figures.rs}` |
...
| H-090 | Type gamma distance-to-agreement and search radius as Aequitas `Length<T>`, global/local dose criteria and pass-rate thresholds as `AbsorbedDose<T>`, and migrate every analysis/simulation caller and book snippet. Preserve Low gamma values, local/global normalization, grid identity, and scalar pass-rate semantics. | [arch] [major] | done | Codex | `crates/helios-analysis/**`, `crates/helios-simulation/**`, `docs/adr/0007-gamma-physical-criteria.md`, `docs/book/**`, PM artifacts |
| H-091 | Carry Aequitas `EnergyPerArea<T>` through helical delivery, portal fluence, and delivered-dose geometry; type couch position, leaf width, sampling step, standoff, and point-source axis as `Length<T>`. Convert to the millimetre-based voxel/ray kernel only at the numerical boundary and preserve scalar dense-volume storage. Verify closed-leaf zero, attenuation monotonicity, fluence linearity, geometry limits, f32 instantiation, all examples, and the end-to-end delivery workflow. | [arch] [major] | done | Codex | `crates/helios-simulation/**`, `docs/adr/0008-delivery-physical-quantities.md`, `docs/book/**`, PM artifacts |
...
| H-119 | Carry required DICOM `BitsStored` through Helios's RITK pixel layout, reject missing, out-of-range, and multi-valued metadata at the input boundary, and advance the standalone lock to the merged provider contract. | [patch] | in-progress | Codex `/root/fit` | `crates/helios-domain/src/dicom.rs`, `Cargo.lock`, `backlog.md` |
...
| H-041 | End-to-end workflow validation: one integration test (`helios-simulation/tests/end_to_end.rs`) where a shared μ drives both the imaging branch (Radon→FBP→registration) and the therapy branch (helical MLC delivery→divergent-fan dose→scatter→DVH/gamma). Oracles are analytical: the gamma runs against a uniformly scaled comparison field whose peak gamma and pass rate are derived in closed form, with a 6%-low negative control that must fail (a dose-against-itself gamma is 100% by construction and validates nothing). | [minor] | done | claude-helios | `crates/helios-simulation/tests/**` |
| H-041b | Runnable example `helios-simulation/examples/tomotherapy_workflow.rs`: CT→μ→(Radon/FBP recon) + (helical MLC delivery→divergent-fan dose→scatter)→DVH/gamma, rendering `ct/mu/recon/dose.png` (Output & visual verification — inspected: phantom, FBP recovery, central rotational dose falloff). Prints recon μ +0.1% and the 3%/2 mm pass rate against a 2%-low comparison field alongside its 6%-low negative control (the example previously reported a dose-against-itself gamma, which is 100% by construction). | [minor] | done | claude-helios | `crates/helios-simulation/examples/**` |
