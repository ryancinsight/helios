...
| H-060 | Coverage tooling (G-17): `lld` linker unblocks the instrumented build (`RUSTFLAGS=-Clink-arg=-fuse-ld=lld`); residual `cargo llvm-cov` region-attribution is empty on the GNU target → try `grcov`, or MSVC/Linux CI, to obtain the coverage % | [patch] | todo | — | CI/tooling | 
