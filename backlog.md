...
| H-021b | `helios-simulation`: moirai-parallel projection dispatch (via `map_collect_index_with::<Adaptive>`); fan/cone-beam detector rows; motion. **BLOCKED (concurrent-agent):** a peer's uncommitted `mnemosyne-arena/tagged_stack.rs` WIP does not compile (E0061); `moirai-parallel → moirai-executor → mnemosyne-arena` pulls it, so the moirai consumption cannot build/verify until the peer commits a green fix. Change designed + reverted to keep helios green; retry when the cluster compiles. | [minor] | blocked | — | `crates/helios-simulation/**` |
