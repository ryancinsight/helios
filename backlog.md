...
| H-004c | `helios-domain`: multi-slice DICOM **series** stacking → 3-D `Volume` (sort by `ImagePositionPatient`, derive z spacing, validate consistent geometry); `CtVolume`/`MvctVolume` HU-semantic newtypes; `ImageOrientationPatient` → oriented grid pose (pairs with H-003d) | [minor] | todo | — | `crates/helios-domain/**` |
