...
use helios_physics::{
    ...
    thomson_cross_section, MassAttenuation,
};
...
fn main() {
    ...
    let labels = ["10 keV", "50 keV", "100 keV", "500 keV", "1 MeV",
                  "6 MeV (6MV)", "10 MeV", "18 MeV (18MV)"];
    ...
    let sigmas: Vec<f64> = energies_mev.iter().map(|&e| klein_nishina_cross_section(e)).collect();
    let monotone = sigmas.windows(2).all(|w| w[0] > w[1]);
    assert!(monotone, "KN cross section must decrease monotonically with energy");
    ...
    for &(label, energy) in &[("100 keV", 0.1_f64), ("6 MeV", 6.0_f64)] {
        ...
        println!("  {:<14}  μ_C/ρ = {:.4} cm²/g  (water)", label, mu_rho.get());
    }
