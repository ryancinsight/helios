...
| `helios-solver` | GPU-accelerated deterministic dose engines (collapsed-cone / convolution-superposition) and imaging forward projectors/reconstruction. Landed (0.0.1): HU→μ engine, `forward_project_ray` (∫μ dl), primary-fluence transport + convolution dose, `deposit_ray_terma` (primary-energy terma deposition with exact `w·(1−e^{−τ})` conservation). Divergent fan + lateral scatter kernel + GPU backend pending. | **partial (0.0.1)** |
