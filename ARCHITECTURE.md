...
| `helios-imaging` | MVCT acquisition modeling, reconstruction, portal dosimetry, IGRT workflows. Landed (0.0.1): parallel-beam Radon forward transform (`Sinogram`), Ram-Lak FBP + SIRT iterative reconstruction (shared back-projector), and a deterministic quantum-noise model (`add_quantum_noise`). Portal dosimetry + IGRT registration (ritk) pending. | **partial (0.0.1)** |
