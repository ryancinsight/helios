...
| `helios-solver` | GPU-accelerated deterministic dose engines (collapsed-cone / convolution-superposition) and imaging forward projectors/reconstruction. Landed (0.1.0): HU→μ engine, rigid-pose-aware `forward_project_ray` (∫μ dl) and `deposit_ray_terma`, primary-fluence transport, `scatter_superposition` (separable 3-D kernel superposition → lateral penumbra + build-up), `anisotropic_scatter_superposition` (grid-axis forward-peaked kernel), `oriented_forward_scatter` (forward-peaked collapsed cone along an **arbitrary** beam direction via trilinear `directional_convolve`), and `poly_forward_peaked_kernel` (energy-fluence-weighted beam-hardened kernel spectra, `SpectralComponent`). Per-leaf gaia collimation pending (H-020k). | **partial (0.1.0)** |
