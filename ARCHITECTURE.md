...
|-----------------|-------------------|-------------|-------------------|
| **ritk** | `ritk-dicom` (**consumed**, feature `dicom`), `ritk-registration` (**consumed**), `ritk-core` | domain, analysis, imaging | Sole Helios-facing DICOM boundary (CT/MVCT parse, typed attributes, transfer-syntax dispatch, and rescale-calibrated decode); Helios has no direct dicom-rs dependency. IGRT integer-voxel translation registration (`classical::translation`, SSD + NCC metric policies) is consumed by `helios-imaging`, which re-exports the kernel's typed error. `ritk-core` is declared but consumed by no member. RT struct/plan/dose, sub-voxel/deformable registration, and VTK pending. |
